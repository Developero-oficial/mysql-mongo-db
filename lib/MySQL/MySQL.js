  ...
  generate (params) {
    ...
    for (let property in params) {
      for (let op in operators) {
        if (params[property].search(op)) {
          valueToEscape = params[property].substring(params[property].search(operators[op]) + 1, params[property].length)
          valueToExtract = params[property].substring(0, params[property].search(operators[op]) + 1)
          break
        }
      }
      if (property.toString().search('_')) {
        property = property.toString().replace('_', ' ')
      }
      if (valueToEscape || valueToExtract) {
        sentence += `${property} ${valueToExtract.trim()}${this.conn.escape(valueToEscape.trim())} `
      } else {
        sentence += `${property} ${params[property]} `
      }
    }
    return sentence
  }
